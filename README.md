Alibaba Cloud ALB Cross-Region Load Balancing Terraform Module

# terraform-alicloud-alb-cross-region-load-balancing

English | [简体中文](https://github.com/alibabacloud-automation/terraform-alicloud-alb-cross-region-load-balancing/blob/main/README-CN.md)

Terraform module for implementing the solution [Implementing Cross-Region Load Balancing with ALB](https://www.aliyun.com/solution/tech-solution/alb-acrlb). It involves the creation, configuration, and deployment of resources such as Virtual Private Cloud (VPC), VSwitch, Elastic Compute Service (ECS), Cloud Enterprise Network (CEN), and Application Load Balancer (ALB).

## Usage

This module creates a complete ALB cross-region load balancing architecture with CEN, Transit Routers, and backend ECS instances distributed across multiple regions. The ALB distributes traffic to backend servers in different regions through the CEN network.
...
module "alb_cross_region" {
  ...
  # Region 1 network configuration (ALB deployment region)
  ...
  # Region 2 network configuration (Backend region)
  ...
  # Region 3 network configuration (Backend region)
...
* [Complete Example](https://github.com/alibabacloud-automation/terraform-alicloud-alb-cross-region-load-balancing/tree/main/examples/complete)
